...
export default function Desktop() {
  // Desktop configuration list with fully verified WordPad document asset for Resume
  const systemLaunchers: LauncherItem[] = [
    ...
    // FIXED LINE BELOW: Swapped out broken URL for the guaranteed WordPad rich text link
  ...
  // Primary Window Open Visibility Tracking
  ...
  // Easter Egg System Repositories
  ...
  // Tracks system clock intervals client-side
  useEffect(() => {
    const updateClock = () => {
      const now = new Date();
  ...
  const launchApplication = (app: LauncherItem) => {
    ...
    setMinimizedWindows(prev => ({ ...prev, [app.id]: false })); // Force un-minimize frame layer on relaunch
    setActiveWindow(app.id);
  };

  // Handles managing taskbar actions: minimizes active frames, sets focal tabs, or un-minimizes hidden containers
  const handleTaskbarClick = (id: string) => {
    if (activeWindow === id && !minimizedWindows[id]) {
      setMinimizedWindows(p => ({ ...p, [id]: true })); // Clicking active taskbar element forces a minimize action
    } else {
      setMinimizedWindows(p => ({ ...p, [id]: false })); // Unhide minimized windows cleanly
      setActiveWindow(id);
    }
  };
  ...
  return (
    <main className="relative w-full h-screen flex flex-col justify-between bg-[#000080] overflow-hidden">
      ...
      <div className="relative flex-1 p-6 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center p-4 sm:p-8 z-20">
          ...
          <Window 
            ...
            bodyHeight="h-[72vh] max-h-[580px]" // Structural height restriction constraint to generate scrollbars
          >
            <ResumeViewer />
          </Window>
      ...
      <div className="h-10 bg-[#c0c0c0] border-t-2 border-white flex items-center justify-between px-1 z-50 select-none">
        ...
        <div className="px-2 py-1 bg-gray-300 text-xs text-black font-mono flex items-center gap-1.5 h-7 shadow-inner" style={{ borderStyle: 'inset', borderWidth: '2px' }}>
          <img src="https://win98icons.alexmeub.com/icons/png/loudspeaker_rays-0.png" className="w-3.5 h-3.5 object-contain" alt="Volume" />
          <span>{time}</span>
        </div>
      </div>
